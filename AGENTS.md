...
Trip records may include an optional `image` field. Use a path such as `Images/conference-group.jpg`; the map will show it inside that trip's popup. Keep image files in `Images/`.
